...
PleC::PleC():
  ...
  ph2k_CckA(CckA_phos, CckA_kin, DivL, 10.0, first_order, 0.5*scale),
...
////}
////
////void PleC::printFirings(char filename[]){
////
////  ofstream ofile(filename, ios::out|ios::app);
////
////  for(int i=0; i<firings.size(); i++)ofile<<firings[i]<<endl;
////
////  ofile.close();
////}
////
...
void PleC::printPopulation(char filename[]){
...
void PleC::printPropensity(char filename[]){
...
void PleC::SSA(double t, double timeStep){
  ...
  while(timeTracker < t){
    ...
    while(timeTracker > Timetag){
      printPopulation();   Timetag += timeStep;
    }
